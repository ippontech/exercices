...
class AssertUnitTest {
  ...
  @Test
  void shouldValidateBadFormatWithNullInput() {
    assertThatCode(() -> Assert.field(FIELD_NAME, (String) null).format("\\d*", "9 character")).doesNotThrowAnyException();
  }

  @Test
  void shouldValidateBadFormat() {
    assertThatCode(() -> Assert.field(FIELD_NAME, "value").format("\\d*", "9 character"))
      .isExactlyInstanceOf(StringBadFormatException.class)
      .hasMessageContaining("9 character");
  }

  @Test
  void shouldValidateGoodFormat() {
    assertThatCode(() -> Assert.field(FIELD_NAME, "123456").format("\\d*", "9 character")).doesNotThrowAnyException();
  }
