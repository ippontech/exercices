...
public final class CucumberTestContext {
  ...
  private static UUID lastDummyId;
  ...
  public static void setLastDummyId() {
    lastDummyId = UUID.fromString((String) CucumberTestContext.getElement("$.id"));
  }

  public static UUID getLastDummyId() {
    return lastDummyId;
  }
  ...
  private static class RestQuery {
    ...
    private Optional<String> readResponse(ClientHttpResponse response) {
      try {
        return Optional.of(StreamUtils.copyToString(response.getBody(), StandardCharsets.UTF_8));
      } catch (Exception e) {
     ...
     * </pre>
     *
     * @param uri name of a REST resource, such as "working-folders"
     */
