...
public class Assert {
  ...
  public static class StringAsserter {
    ...
    /**
     * Ensure that the given input value is not at the wrong format
     *
     * @param regex          format of the value (it won't be in message) {@link String}
     * @param acceptedValues accepted values (it will be in message) {@link String}
     * @return The current asserter
     * @throws StringTooLongException if the value is longer than the max length
     */
    public StringAsserter format(String regex, String acceptedValues) {
      if (value == null) {
        return this;
      }

      if (!value.matches(regex)) {
        throw StringBadFormatException.builder().field(field).value(value).acceptedValues(acceptedValues).build();
      }
